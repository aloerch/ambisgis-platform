...
public class ImageIOWitness {
 ...
 static void roundtrip(Path output,String format,boolean own,boolean geotiff)throws Exception {
  ...
  if(param instanceof J2KImageWriteParam){((J2KImageWriteParam)param).setLossless(true);((J2KImageWriteParam)param).setFilter("w5x3");}
  ...
  try(ImageInputStream stream=ImageIO.createImageInputStream(file.toFile())) {
   ...
   ImageReader r=readers.next();if(own){r.dispose(); r = format.equals("TIFF") ? new com.sun.media.imageioimpl.plugins.tiff.TIFFImageReaderSpi().createReaderInstance() : new com.sun.media.imageioimpl.plugins.jpeg2000.J2KImageReaderSpi().createReaderInstance();}
   try {r.setInput(stream);BufferedImage result=r.read(0);if(result.getWidth()!=96||result.getHeight()!=80)throw new AssertionError("dimensions");long delta=0;int max=0;
    for(int y=0;y<80;y++)for(int x=0;x<96;x++){int a=input.getRGB(x,y),b=result.getRGB(x,y);for(int shift=0;shift<24;shift+=8){int diff=Math.abs(((a>>>shift)&255)-((b>>>shift)&255));if(delta==0 && diff>0 && format.equals("JPEG2000"))System.out.println("FIRST_JP2_DIFFERENCE x="+x+" y="+y+" expected="+Integer.toHexString(a)+" actual="+Integer.toHexString(b)+" raster="+Arrays.toString(input.getRaster().getPixel(x,y,(int[])null))+"/"+Arrays.toString(result.getRaster().getPixel(x,y,(int[])null)));delta+=diff;max=Math.max(max,diff);}if(alpha&&(a>>>24)!=(b>>>24))throw new AssertionError("alpha changed");}
...
Path out=Path.of(args[0]);Files.createDirectories(out);ImageIO.setUseCache(false);inventory();for(String f:new String[]{"PNG","JPEG","TIFF"})roundtrip(out,f,false,false);roundtrip(out,"TIFF",true,true);for(String c:new String[]{"CCITT RLE","CCITT T.4","CCITT T.6"})fax(out,c);roundtrip(out,"JPEG2000",false,false);roundtrip(out,"JPEG2000",true,false);System.out.println("IMAGEIO_WITNESS_PASS");}
}
