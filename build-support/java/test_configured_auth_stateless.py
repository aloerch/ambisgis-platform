...
import configured_auth_stateless as repairs
...
class ConfiguredStatelessRepairTests(unittest.TestCase):
    def setUp(self):
        ...
        self.spec = {'sources': [{'path': 'source.java', 'purpose': 'stateless',
    ...
    def test_baseline_injects_native_tests_without_repair(self):
        ...
        self.assertFalse(receipt['default_browser_behavior_changed'])
    ...
    def test_packaging_repairs_sources_without_test_injection(self):
        ...
        self.assertTrue(receipt['authentication_decision_changed'])
        self.assertEqual(receipt['requires_explicit_filter_option'], 'statelessBearerAuthentication')
        self.assertEqual(receipt['required_preceding_repairs'][-1], 'configured-auth-repairs.json')
