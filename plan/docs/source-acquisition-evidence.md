...
## Proposed PostgreSQL 15.19 candidate probe

The [candidate tuple proposal](candidate-tuple-evidence.md) derives a PostgreSQL 15/PostGIS 3.5 family from the retained GeoNode 5.1.0 source declarations. The exact owned PostgreSQL `REL_15_19` commit `2ff1375b5dd8bf09d8cb0e795974528180fd75ca` was separately audited and built on local branch `fnd-02/postgresql15-build`. Unchanged default configure and `make -j4` passed; `make -j4 check` passed **217 actual core tests**, with zero failure or skip markers. Source stayed clean and the temporary cluster shut down and was removed. No global install or remote change occurred.

[The normalized report](../verification/postgresql15-candidate-build.json) records actual commands, inputs, artifacts and log hashes; original report SHA256 is `04a773f6451ea2088f39b9afb114d5ba88a7a9a8de65f4544ea1c1d8d9310812`. Configure took 6.930 seconds, compilation 43.473 seconds and core checks 5.821 seconds in this environment. These observations establish candidate core-build feasibility, not performance targets or tuple acceptance.

PostgreSQL 15's unchanged default profile leaves SSL, ICU, XML, LLVM and TAP disabled; readline and zlib are enabled. These defaults were not introduced to suppress failing tests. A reviewed product feature profile, PostGIS integration/install/upgrade/raster/vector-tile tests, the remaining owned components, retained toolchain closure, disconnected rebuild and independent repair remain outstanding. The proposed source tags are not approved canonical product baselines.


After integrating the owner-approved GOV-01 merge `3e7581a06b000856ea9b27468cc415612c28b4b0`, this source-evidence branch passed **143 package/governance tests** and strict plan/schema validation from `plan/`. [Final test output](../verification/source-evidence-tests.txt) and [schema output](../verification/source-evidence-schemas.txt) are retained. An earlier invocation from the repository root failed to import six test modules because `tools` is under `plan/`; it did not execute those tests and was corrected by using the documented working directory. The passing rerun has no failures or skips. This does not change the separate owned-engine or GIS acceptance limits above.
