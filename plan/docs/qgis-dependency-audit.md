...
Keep `BUILD_WITH_QT6=OFF`, `WITH_DESKTOP=ON`, `WITH_GUI=ON`, `WITH_PYTHON=ON`, `WITH_BINDINGS=ON`, `WITH_SERVER=ON`, `WITH_SERVER_PLUGINS=ON`, `WITH_POSTGRESQL=ON`, `WITH_SPATIALITE=ON`, `WITH_ANALYSIS=ON` and `ENABLE_TESTS=ON`. Optional exclusions supported by exact source options include `WITH_3D`, `WITH_PDAL`, `WITH_DRACO`, `WITH_GRASS7`, `WITH_GRASS8`, `WITH_QTWEBKIT`, `WITH_QTWEBENGINE`, `USE_OPENCL` and `WITH_SERVER_LANDINGPAGE_WEBAPP`; these exclude 3D/PDAL/GRASS/embedded browser/GPU/landing-page UI capabilities and cannot be advertised by this profile. `WITH_INTERNAL_SPATIALINDEX=ON` uses retained embedded source and avoids the candidate's explicit rejection of external spatialindex >=2.1. Actual compilation found that disabling both COPC and EPT is not coherent in this unmodified candidate. The corrected profile enables `WITH_COPC=ON`, `WITH_EPT=ON` and `WITH_INTERNAL_LAZPERF=ON`, with retained ZSTD 1.5.7 development/runtime inputs; PDAL and 3D remain disabled. COPC/EPT are selected for compilation and remain unexercised in this checkpoint. Their inclusion is not point-cloud product acceptance. Final chosen options remain the build receipt's authority.
...
| zlib | 1.3.1-3.3 | Zlib |
| zstd | 1.5.7-6.1 | BSD-3-Clause AND GPL-2.0-only |

The table copies publisher classification; it does not resolve all combination, redistribution, notice, trademark or source-offer obligations. Original RPMs and source archives are retained unchanged. The extracted private prefix retains publisher notices under `usr/share/licenses`; the final selected support-07 prefix contains 338 notice files. A broader source bootstrap and host runtime closure remain later gates, as authorized by this bounded checkpoint.
...
The host executes these tools through `flatpak-spawn --host` where required. That bridge is not hostile-code isolation. Extraction prefixes are task-owned. The final selected supporting prefix is `build-worktrees/qgis-candidate/support-07/usr`; its adjacent `support-07-inventory.json` records every file hash and symlink. The existing `support`, `support-02`, `support-03`, `support-04`, `support-05`, `support-06` prefixes and unsuccessful/incomplete extraction snapshots are preserved. The selected inventory contains 13,469 entries and has SHA256 `aa837efdf41201593ba6d2a48ec7f8320fd8a70c3f456075503fe1f08dd8713a`; its embedded support manifest has SHA256 `7636a34b1823e071dc9662522d8f79be3085953eaed2d64ad6fcd34d1cb37d47`. Compact acquisition, metadata, source/notice, preflight and limitation evidence is in `plan/verification/qgis-candidate/support.json`.
...
The selected SpatiaLite library also required `xmlNanoHTTPCleanup`, absent from the original retained libxml2 2.14.6 profile built with HTTP disabled. `build-support/qgis/xml_profile.py` rebuilds the exact retained archive, unchanged, into `build-worktrees/qgis-candidate/xml-profile-01/prefix`, enabling HTTP while keeping Python/LZMA/iconv off and using original native zlib. The 48-step GCC15 build exports the required symbol. A complete `RTLD_NOW` SpatiaLite 5.1.0 load succeeds with exactly the new XML library, spatial-04 SQLite/PROJ, original native GEOS/zlib and supporting SpatiaLite origins. All 3,913 extracted source files and existing native/spatial/support inventories remain unchanged; the new prefix has 116 inventoried files/links. All five producer/probe phases retain the existing offline syscall restrictions. Restoring an API does not authorize external fetching or waive application network controls. Exact source, producer/helper snapshots, output and loaded-origin hashes are recorded in the `xml_profile` section of the compact support receipt.
...
## Bounded source notice review for F02-06

All 133 retained QGIS source notice contents were checked against the owned-source manifest and scanned for concrete resource terms. The follow-up traced selected files through `resources/CMakeLists.txt:4`, `cmake/CopyResources.cmake:39` and the actual build-04 generated install rules. Those rules include every palette counted below and do not interpret the notices' `distribute` metadata. These observations identify review questions; they are not a license-clearance conclusion or evidence that installation has completed.

| Selected source resource | Concrete retained notice observation | F02-06 question |
|---|---|---|
| `resources/cpt-city-qgis-min/jjg/ccolo/*/COPYING.xml` | Nineteen notices name CC Attribution-Noncommercial-Share Alike 3.0 and mark `distribute="noncomm"`; 256 SVGs are selected for installation. | Decide whether to omit/replace these assets or establish suitable permission for intended product distribution. |
| `resources/cpt-city-qgis-min/jjg/neo10/COPYING.xml` and `td/COPYING.xml` | Both mark `distribute="no"` and say only “Free to use”; td adds “None specified.” Their directories contain 135 and 3 selected SVGs. | Resolve an adequate redistribution grant or select replacements. |
| `resources/cpt-city-qgis-min/es/COPYING.xml` | 690 selected SVGs carry custom resource-redistribution conditions concerning credit, free sharing, original preview/name preservation and merged collections. A historical author-approval remark is present but its external source was not verified. No PNG/JPEG previews were found in this subtree. | Determine whether retained resource/preview/credit forms and intended distribution satisfy the actual permission. |
| `resources/cpt-city-qgis-min/jm/COPYING.xml` | The short label says attribution; the full text applied to 45 selected palettes describes personal/nontransferable rights and derivative works for clients' internal use. | Resolve applicability and redistribution scope from the full agreement. |
| `images/themes/default/LICENSE.TXT` | CC BY-SA 3.0 notice requests including the license file. Default icons compile through `images/images.qrc` into core; this exact notice is neither in that qrc nor referenced by generated install rules. The contributor list does name Robert Szczepanek. | Establish the product notice/attribution mechanism; absence of this exact file is not a claim that all other mechanisms are absent. |
| `resources/cpt-city-qgis-min/cb/COPYING.xml` | The 265 selected ColorBrewer SVGs have specific acknowledgement and endorsement/product-naming terms under an Apache-style Version 1.1 text. | Map those exact terms to product acknowledgement and brand review. |

The `source_notice_review` section of `plan/verification/qgis-candidate/support.json` retains exact notice paths/hashes, each of the nineteen noncommercial notice hashes/counts, representative asset hashes and generated-install evidence. The retained QGIS code license has its own Qt linking exception; that code exception does not describe these separate resource terms. No source assets or terms were changed, and no external author was contacted.
